...
async def my_profile_settings(
    ...
    async with async_session_maker() as async_session:
        try:
            ...
            if not user_record:
                return {"state": 404, "detail": "User not found"}
            if user_update.email and user_update.email != user_record.email:
                ...
                if existing_user:
                    return {"state": 400, "detail": "A user with this email already exists"}
            ...
            return {"state": 500, "detail": f"An unexpected error occurred: {str(e)}"}
