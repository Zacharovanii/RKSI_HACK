...
from src.vacancy.schemas import VacancyCreateModel, VacancyReadModel
...
async def edit_vacancy(id: int, new_vacancy: VacancyCreateModel, user: User = Depends(current_user)) -> dict:
    async with AsyncSession(async_engine) as session:
        user = await session.get(User, user.id)
        vacancy = await session.get(Vacancy, id)
